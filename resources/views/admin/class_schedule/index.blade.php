...
@section('title', settings()->website_title . ' | ' . __('words.class_schedules'))
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <span class="text-muted">{{ __('words.show_class_schedules') }}</span>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap py-5">
            <div class="card-title">
                <h3 class="card-title">{{ __('words.show_class_schedules') }}</h3>
            </div>
            ...
            <table class="table table-separate table-head-custom table-checkable" id="custom_datatable">م                <thead>
                ...
                @foreach ($schedules as $key => $schedule)
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <a href="{{$schedule->cover}}"
                                   data-toggle="lightbox"
                                   data-gallery="gallery">
                                    <img class="index_image" src="{{ $schedule->cover }}"
                                         onerror="this.src='{{ asset('uploads/default_image.png') }}'" alt="class_schedule-image">
                                </a>
                            @endif
                        </td>
                        <td>
                            ...
                            @else
                                <a href="{{$schedule->image}}"
                                   data-toggle="lightbox"
                                   data-gallery="gallery">
                                <img class="index_image" src="{{ $schedule->image }}"
                                     onerror="this.src='{{ asset('uploads/default_image.png') }}'" alt="class_schedule-image">
                                </a>
                            @endif
                        </td>
                        ...
                        <td>{{ formatDate($schedule->created_at) == formatDate($schedule->updated_at) ? '--' : formatDate($schedule->updated_at) }}
                        </td>
                        <td nowrap="nowrap">
                            @include('admin.components.form-controls', [
                                'name' => 'schedules',
                                'value' => $schedule,
                                'role' => 'class_schedules',
                            ])
                        </td>
                    </tr>
                @endforeach
