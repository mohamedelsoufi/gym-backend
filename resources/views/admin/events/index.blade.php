...
@section('title', settings()->website_title . ' | ' . __('words.events'))
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <span class="text-muted">{{ __('words.show_events') }}</span>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap py-5">
            <div class="card-title">
                <h3 class="card-title">{{ __('words.show_events') }}</h3>
            </div>
            ...
            <table class="table table-separate table-head-custom table-checkable" id="custom_datatable">
                ...
                @foreach ($events as $key => $event)
                    <tr>
                        ...
                        <td>{{ date('h:i A',strtotime($event->from)) }}</td>
                        <td>{{ date('h:i A',strtotime($event->to)) }}</td>
                        <td><span
                                class="badge rounded-pill text-white {{$event->status == 1 ? 'bg-success' : 'bg-danger'}}">{{ $event->getActive() }}</span>
                        </td>
                        <td>{{ formatDate($event->created_at) }}</td>
                        <td>{{ formatDate($event->created_at) == formatDate($event->updated_at) ? '--' : formatDate($event->updated_at) }}
                        </td>
                        <td nowrap="nowrap">
