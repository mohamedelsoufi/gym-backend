...
@section('title', settings()->website_title . ' | ' . __('words.create_class'))
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <span class="text-muted">{{ __('words.create_class') }}</span>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
...
@section('form_action', route('classes.store'))
    ...
    <div class="card card-custom mb-2">
        <div class="card-header card-header-tabs-line">
            <div class="card-title">
                <h3 class="card-label">{{ __('words.create_class') }}</h3>
            </div>
                        ...
                        <div class="input-group date">
                            <select class="form-control selectpicker" id="multiSelect1" multiple="multiple"
                                ...
                                @foreach($all_days as $day)
                                    <option
                                        value="{{$day->id}}" {{(collect(old('days'))->contains($day->id)) ? 'selected':'' }}>{{$day->day}}</option>
                                @endforeach
                            </select>
