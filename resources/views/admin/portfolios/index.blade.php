...
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <span class="text-muted">{{ __('words.show_portfolios') }}</span>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
            ...
            <table class="table table-separate table-head-custom table-checkable" id="example1">
                ...
                @foreach ($portfolios as $key => $portfolio)
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <a href="{{$portfolio->image}}"
                                   data-toggle="lightbox" data-title="{{$portfolio->title}}"
                                   data-gallery="gallery">
                                    <img class="index_image" src="{{ $portfolio->image }}"
                                         onerror="this.src='{{ asset('uploads/default_image.png') }}'"
                                         alt="portfolio-image">
                                </a>
                            @endif
                        ...
                        <td><span
                                class="badge rounded-pill text-white {{$portfolio->status == 1 ? 'bg-success' : 'bg-danger'}}">{{ $portfolio->getActive() }}</span>
                        </td>
